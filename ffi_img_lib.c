...
struct image_t* load_image(const char* path) {
	FILE* fp = NULL;
	struct image_t* out = NULL;
	png_struct* png = NULL;
	png_info* info = NULL;
	
	if (!(out = malloc(sizeof(struct image_t))))
		abort();
...
struct image_t* copy_image(struct image_t* src) {
	...
	struct image_t* out = malloc(sizeof(struct image_t));
	if (!out)
		abort();
	...
	size_t row_sz = sizeof(png_byte) * out->width * out->channels;
	for (int y = 0; y < out->height; ++y) {
		if (!(out->rows[y] = malloc(row_sz)))
			abort();
		memcpy(out->rows[y], src->rows[y], row_sz);
	}
